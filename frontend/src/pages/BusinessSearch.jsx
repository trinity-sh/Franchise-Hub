  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        "https://franchise-hub-server.herokuapp.com/api/v1/webview/section/filter/all" +
          location.search
      );
      ...
      <>
        <div className="container business">
          <Card className="business-card">
            <Card.Header>
              <Card.Title>
                {item.content.personal_details.brand_name}
              </Card.Title>
            </Card.Header>
            <Card.Body>
              <Row>
                <Col>
                  <span>
                    About Us: {item.content.personal_details.business_desc}
                  </span>{" "}
                  <br />
                  <span>Country: {item.content.personal_details.country}</span>
                  <Link to={"/brands/" + `${item._id}`} state={{ prop: item }}>
                    <Button variant="outline-danger" className="button mt-4">
                      Know More
                    </Button>
                  </Link>
                </Col>
